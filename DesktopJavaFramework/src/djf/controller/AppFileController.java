...
public class AppFileController {
    ...
    public void markAsUnEdited(AppGUI gui) {
        // THE WORK IS NOW DIRTY
        saved = true;
     ...
     */
    public void handleNewRequest(boolean withGui) {
        ...
        try {
            ...
            if (continueToMakeNew) {
                ...
                if (!app.isTesting()) {
                    if (withGui) {
                        dialog.show(props.getProperty(NEW_COMPLETED_TITLE), props
                          .getProperty(NEW_COMPLETED_MESSAGE));
                        app.getGUI().updateToolbarControls(true);
                    }
                } else {
                    dialog.show("Test Save Complete", "Test Save successfully completed the save");
                }
                synchronized (app.getGUI().lock) {
                    app.getGUI().lock.notify();
                    unlocked = true;
        ...
        if (selection.equals(AppYesNoCancelDialogSingleton.YES)) {
            ...
            if (currentWorkFile == null) {
                ...
                fc.getExtensionFilters().addAll(
                  new ExtensionFilter(props.getProperty(WORK_FILE_EXT_DESC), props
                    .getProperty(WORK_FILE_EXT)));
    ...
    private void promptToOpen() {
        ...
        if (selectedFile != null) {
            try {
                ...
                app.getFileComponent()
                   .loadData(app.getDataComponent(), selectedFile.getAbsolutePath());
...
//        File exportDirectory = f.showDialog(app.getGUI().getWindow());
//
//        if (exportDirectory == null) {
//            return;
//        }
//
//        String r = Paths.get("").toAbsolutePath().toString();

        try {
            app.getFileComponent()
               .exportData(app.getDataComponent());//, exportDirectory.toString());
    ...
    public void copyAllIn(File directory, File to) {
        boolean wasSaved = false;
        File[] files = directory.listFiles();
        for (File file : files) {
            File newFile = new File(
              to.toString() + File.separator + file.getName());
            try {
                if (newFile.toString().contains("OfficeHoursGridData")) {
                    if (this.saved) {
                        wasSaved = true;
                    }
                    this.saveWork(newFile);
                    if (!wasSaved) {
                        this.markAsEdited(app.getGUI());
                    }
                ...
                AppMessageDialogSingleton.getSingleton()
                                         .show("Export Error", "Some files could not be creataed during the export process.");
            }
            if (file.isDirectory()) {
                copyAllIn(file, new File(
                  to.toString() + File.separator + file.getName()));
            }
